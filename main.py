...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "action" in context.user_data:
        if context.user_data["action"] == "add_admin":
            if update.message.reply_to_message:  # 必须回复用户消息
                ...
                try:
                    ...
                    if member.status in ["member", "administrator", "creator"]:
                        ...
                        await update.message.reply_text(f"❌ 用户 @{target_user.username} 不在本群")
                except:
                    await update.message.reply_text("❌ 无法验证该用户，请确保他在群里")
            else:
                await update.message.reply_text("❌ 请回复用户的消息进行添加")
