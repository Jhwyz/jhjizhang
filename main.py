...
async def set_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if "%" in text:
            rate = float(text.replace("%","").split()[-1])
        else:
...
async def set_exchange(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user not in data["admins"]:
        return
    try:
        exchange = float(update.message.text.split()[-1])
...
# 启动 Webhook
app.run_webhook(listen="0.0.0.0", port=8443, url_path=TOKEN, webhook_url=WEBHOOK_URL+TOKEN)
