...
import requests  # ✅ 用于获取 OKX C2C 数据
...
async def z0(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, (price, name) in enumerate(orders, start=1):
        msg += f"{i}️⃣ 价格：{price} 元 — 商家：{name}\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------- 计算器功能 ----------
    calc_match = re.fullmatch(r'[\d\s\.\+\-\*/\(\)]+', text)
    if calc_match:
        try:
    ...
    if "action" in context.user_data:
        # 原 handle_message 中管理员逻辑保持不变
        ...

# ---------- 菜单 ----------
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("📊 实时报价 (OKX USDT)", callback_data="okx_price")],
...
app.add_handler(MessageHandler(filters.Regex("^z0$"), z0))
